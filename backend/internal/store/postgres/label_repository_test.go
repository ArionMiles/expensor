...
func TestLabelRepositoryCRUD(t *testing.T) {
	...
	if err := ts.CreateLabel(ctx, testTenant(t, ts), "custom-test", "#38bdf8"); err != nil {
		t.Fatalf("CreateLabel: %v", err)
	}
	labels, err := ts.ListLabels(ctx, testTenant(t, ts))
	if err != nil {
	...
	if err := ts.UpdateLabel(ctx, testTenant(t, ts), "custom-test", "#f97316"); err != nil {
	...
	if err := ts.DeleteLabel(ctx, testTenant(t, ts), "custom-test", false); err != nil {
...
func TestDeleteLabelCanRemoveExistingTransactionLabels(t *testing.T) {
	...
	if err := ts.CreateLabel(ctx, testTenant(t, ts), "cleanup-test-preserve", "#6366f1"); err != nil {
		t.Fatalf("CreateLabel preserve: %v", err)
	}
	if err := ts.AddLabel(ctx, testTenant(t, ts), preserveID, "cleanup-test-preserve"); err != nil {
		t.Fatalf("AddLabel preserve: %v", err)
	}
	if err := ts.DeleteLabel(ctx, testTenant(t, ts), "cleanup-test-preserve", false); err != nil {
		t.Fatalf("DeleteLabel preserve: %v", err)
	}
	preserveTxn, err := ts.GetTransaction(ctx, testTenant(t, ts), preserveID)
	if err != nil {
	...
	if err := ts.CreateLabel(ctx, testTenant(t, ts), "cleanup-test-remove", "#6366f1"); err != nil {
		t.Fatalf("CreateLabel remove: %v", err)
	}
	if err := ts.AddLabel(ctx, testTenant(t, ts), removeID, "cleanup-test-remove"); err != nil {
		t.Fatalf("AddLabel remove: %v", err)
	}
	if err := ts.DeleteLabel(ctx, testTenant(t, ts), "cleanup-test-remove", true); err != nil {
		t.Fatalf("DeleteLabel remove: %v", err)
	}
	removeTxn, err := ts.GetTransaction(ctx, testTenant(t, ts), removeID)
	if err != nil {
...
func TestDeleteLabelRemovesMerchantMappings(t *testing.T) {
	...
	if err := ts.CreateLabel(ctx, testTenant(t, ts), "merchant-cleanup", "#6366f1"); err != nil {
		t.Fatalf("CreateLabel: %v", err)
	}
	if _, err := ts.ApplyLabelByMerchant(ctx, testTenant(t, ts), "merchant-cleanup", "Netflix"); err != nil {
		t.Fatalf("ApplyLabelByMerchant: %v", err)
	}

	if err := ts.DeleteLabel(ctx, testTenant(t, ts), "merchant-cleanup", false); err != nil {
		t.Fatalf("DeleteLabel: %v", err)
	}
