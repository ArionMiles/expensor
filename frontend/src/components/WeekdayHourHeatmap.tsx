...
interface Props {
  ...
  monthLabel?: string
  /** Called with the clicked weekday and hour to enable click-through filtering. */
  onCellClick?: (weekday: number, hour: number) => void
}

export function WeekdayHourHeatmap({ data, metric, currency, monthLabel, onCellClick }: Props) {
      ...
      {tooltip && (
        ...
        >
          ...
          <span className="text-muted-foreground">
            {formatCurrency(tooltip.amount, currency)} &middot; {tooltip.count} txn
            {tooltip.count !== 1 ? 's' : ''}
          </span>
        </div>
      )}
