 ...
 * Cells before the start date and after the end date are null (padding).
 * ISO week convention: Mon=0, Sun=6.
 */
function buildCalendarGrid(startDate: Date, endDate: Date): { date: Date | null }[][] {
...
function rollingYearStart(today: Date): Date {
...
export function AnnualCalendarHeatmap({ year, metric, currency, onDayClick }: Props) {
  const [tooltip, setTooltip] = useState<TooltipState | null>(null)

  const today = new Date()
  const isRollingCurrentYear = year === today.getFullYear()
  const currentYearQuery = useAnnualHeatmapData(year)
  const previousYearQuery = useAnnualHeatmapData(year - 1, isRollingCurrentYear)
  const startDate = isRollingCurrentYear ? rollingYearStart(today) : new Date(year, 0, 1)
  const endDate = isRollingCurrentYear ? today : new Date(year, 11, 31)
  const startKey = dateKey(startDate)
  const endKey = dateKey(endDate)
  const annualData = [
    ...(previousYearQuery.data?.buckets ?? []),
    ...(currentYearQuery.data?.buckets ?? []),
  ].filter((bucket) => {
    const key = bucket.date.slice(0, 10)
    return key >= startKey && key <= endKey
  })

  // Build lookup: "YYYY-MM-DD" → DailyBucket
  const lookup = new Map<string, DailyBucket>()
  for (const b of annualData) {
    lookup.set(b.date.slice(0, 10), b)
  }

  const maxValue = Math.max(...annualData.map((b) => (metric === 'amount' ? b.amount : b.count)), 1)
  const grid = buildCalendarGrid(startDate, endDate)
  ...
  if (
    (currentYearQuery.isLoading && !currentYearQuery.data) ||
    (isRollingCurrentYear && previousYearQuery.isLoading && !previousYearQuery.data)
  ) {
    return <div className="h-28 animate-pulse rounded bg-secondary" />
  }

  const isEmpty = annualData.length === 0
  ...
  return (
    <div className="relative">
      ...
      <svg
        ...
        aria-label={`Annual spending heatmap from ${dateKey(startDate)} to ${dateKey(endDate)}`}
      >
        ...
        {grid.map((col, colIdx) =>
          col.map(({ date }, rowIdx) => {
            ...
            const hasData = (bucket?.count ?? 0) > 0
            return (
              <rect
                key={`${colIdx}-${rowIdx}`}
                x={x}
                ...
                style={{ cursor: onDayClick && hasData ? 'pointer' : 'default' }}
                onMouseEnter={(e) =>
                  setTooltip({
                    x: e.clientX,
                    y: e.clientY,
                ...
                onClick={() => {
                  if (onDayClick && hasData) onDayClick(date)
                }}
              />
            )
          }),
      ...
      {tooltip && (
        ...
        >
          ...
          <span className="text-muted-foreground">
            {tooltip.amount > 0
              ? `${formatCurrency(tooltip.amount, currency)} · ${tooltip.count} txn${tooltip.count !== 1 ? 's' : ''}`
              : 'No transactions'}
          </span>
        </div>
      )}
