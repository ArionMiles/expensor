import { clsx, type ClassValue } from 'clsx'
import { twMerge } from 'tailwind-merge'
import type { BankColor } from '@/api/types'
import { formatCurrencyForLocale } from '@/i18n/format'

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs))
}

export function formatCurrency(amount: number, currency: string, locale = 'en-IN'): string {
  return formatCurrencyForLocale(amount, currency, locale)
}

export function formatCompactNumber(value: number): string {
  const abs = Math.abs(value)
  if (abs >= 1_000_000) return `${(value / 1_000_000).toFixed(1)}M`
  if (abs >= 1_000) return `${(value / 1_000).toFixed(1)}K`
  return value.toFixed(1)
}

const READER_DISPLAY_NAMES: Record<string, string> = {
  gmail: 'GMail',
  thunderbird: 'Thunderbird',
}

export function getReaderDisplayName(name: string): string {
  const lower = name.toLowerCase()
  if (READER_DISPLAY_NAMES[lower]) return READER_DISPLAY_NAMES[lower]
  return name
    .split(/[-_\s]+/)
    .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
    .join(' ')
}

// timeFormatToOpts maps a stored time_format value to Intl.DateTimeFormatOptions fields.
function timeFormatToOpts(timeFormat: string): Intl.DateTimeFormatOptions {
  switch (timeFormat) {
    case 'HH:mm:ss':
      return { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }
    case 'h:mm a':
      return { hour: '2-digit', minute: '2-digit', hour12: true }
    case 'h:mm:ss a':
      return { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }
    default: // 'HH:mm'
      return { hour: '2-digit', minute: '2-digit', hour12: false }
  }
}

export function formatDate(
  isoString: string,
  includeTime = false,
  tz?: string,
  timeFormat = 'HH:mm',
  locale = 'en-IN',
): string {
  const date = new Date(isoString)
  const opts: Intl.DateTimeFormatOptions = {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
    ...(tz ? { timeZone: tz } : {}),
    ...(includeTime ? timeFormatToOpts(timeFormat) : {}),
  }
  return new Intl.DateTimeFormat(locale, opts).format(date)
}

export function formatDateShort(isoString: string, tz?: string, locale = 'en-IN'): string {
  const date = new Date(isoString)
  return new Intl.DateTimeFormat(locale, {
    day: '2-digit',
    month: 'short',
    ...(tz ? { timeZone: tz } : {}),
  }).format(date)
}

export function formatRelative(isoString: string): string {
  const date = new Date(isoString)
  const now = new Date()
  const diffMs = now.getTime() - date.getTime()
  const diffSec = Math.floor(diffMs / 1000)
  const diffMin = Math.floor(diffSec / 60)
  const diffHr = Math.floor(diffMin / 60)
  const diffDay = Math.floor(diffHr / 24)

  if (diffSec < 60) return 'just now'
  if (diffMin < 60) return `${diffMin}m ago`
  if (diffHr < 24) return `${diffHr}h ago`
  if (diffDay < 7) return `${diffDay}d ago`
  return formatDate(isoString)
}

const SOURCE_FALLBACK_COLORS = [
  '#3b82f6', // blue
  '#8b5cf6', // violet
  '#ec4899', // pink
  '#f59e0b', // amber
  '#06b6d4', // cyan
  '#14b8a6', // teal
]

// banks may be undefined while the initial fetch is in-flight; falls back to a
// stable hash color for that source string to avoid visual jumps on most renders.
export function getSourceColor(source: string, banks?: BankColor[]): string {
  const lower = source.toLowerCase()
  if (banks) {
    for (const { fragment, color } of banks) {
      if (lower.includes(fragment)) return color
    }
  }
  let hash = 0
  for (let i = 0; i < source.length; i++) {
    hash = (hash << 5) - hash + source.charCodeAt(i)
    hash |= 0
  }
  return SOURCE_FALLBACK_COLORS[Math.abs(hash) % SOURCE_FALLBACK_COLORS.length]
}

const LABEL_COLORS = [
  '#3b82f6', // blue
  '#8b5cf6', // violet
  '#ec4899', // pink
  '#f59e0b', // amber
  '#06b6d4', // cyan
  '#d97706', // orange
  '#6366f1', // indigo
  '#14b8a6', // teal
]

export function getLabelColor(label: string): string {
  let hash = 0
  for (let i = 0; i < label.length; i++) {
    hash = (hash << 5) - hash + label.charCodeAt(i)
    hash |= 0
  }
  const index = Math.abs(hash) % LABEL_COLORS.length
  return LABEL_COLORS[index]
}

export function formatDuration(isoString: string): string {
  const start = new Date(isoString)
  const now = new Date()
  const diffMs = now.getTime() - start.getTime()
  const diffSec = Math.floor(diffMs / 1000)
  const diffMin = Math.floor(diffSec / 60)
  const diffHr = Math.floor(diffMin / 60)
  const diffDay = Math.floor(diffHr / 24)

  if (diffDay > 0) return `${diffDay}d ${diffHr % 24}h`
  if (diffHr > 0) return `${diffHr}h ${diffMin % 60}m`
  if (diffMin > 0) return `${diffMin}m`
  return `${diffSec}s`
}
