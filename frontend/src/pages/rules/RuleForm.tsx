...
function diagnosticFormDefaults(diagnostic: {
  rule_name: string
  sender_email: string
  subject: string
  amount_regex: string
  ...
  return {
    ...
    transactionSource: diagnostic.source,
  }
}
...
export function RuleForm() {
  ...
  useEffect(() => {
    if (rule) {
      setForm({
        ...
        transactionSource:
          rule.source?.label || rule.transaction_source || diagnostic?.source || '',
      })
    }
  ...
  useEffect(() => {
    ...
    setSamples((current) => [diagnostic.email_body, ...current.slice(1)])
    if (isCreate) {
      setForm(diagnosticFormDefaults(diagnostic))
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6 px-6 py-6">
      ...
      {rule?.predefined && (
        <p className="rounded border border-border bg-secondary px-3 py-2 text-xs text-muted-foreground">
          Predefined rule — you can edit it and your changes will persist across restarts.
        </p>
      )}
        ...
        {(
          ...
          <div key={key}>
            ...
            <input
              value={form[key]}
              onChange={set(key)}
              className="w-full rounded border border-border bg-input px-2 py-1.5 font-mono text-xs"
            />
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3 rounded-lg border border-border p-4">
        {diagnostic && (
          <p className="text-xs text-muted-foreground">
            Loaded diagnostic sample from {diagnostic.sender_email || 'email'}.
          </p>
        )}
